...
BLOCK_PATTERN = re.compile(r'<!--\s*Block-Start:\s*(\{.*?\})\s*-->\s*(?P<ticks>`{3,})(\w+)?\s*\n(.*?)\n(?P=ticks)\s*<!--\s*Block-End:\s*(\{.*?\})\s*-->', re.DOTALL)
    ...
    def from_message(cls, message: ChatMessage) -> 'Response':
        ...
        if hasattr(message.message, 'finish_reason') and message.message.finish_reason == 'tool_calls':
            if not self.tool_calls and not errors:
                errors.add("Model finished with 'tool_calls' but no tool calls were found", error_type=ParseErrorType.INVALID_FORMAT)
    ...
    def _parse_code_blocks(self, markdown: str) -> Errors:
        ...
        for match in BLOCK_PATTERN.finditer(markdown):
            ...
            try:
                ...
                errors.add("Invalid JSON in Block-Start", json_str=start_json, exception=str(e), position="Block-Start", error_type=ParseErrorType.JSON_DECODE_ERROR)
                continue
            ...
            try:
                ...
                errors.add("Invalid JSON in Block-End", json_str=end_json, exception=str(e), position="Block-End", error_type=ParseErrorType.JSON_DECODE_ERROR)
                continue
            ...
            if not start_name or start_name != end_name:
                errors.add("Block-Start and Block-End name mismatch", start_name=start_name, end_name=end_name, error_type=ParseErrorType.INVALID_FORMAT)
                continue

            # 创建 CodeBlock 对象，让 Pydantic 处理验证
            try:
                code_block = CodeBlock(name=start_name, lang=lang or "markdown", code=content, path=start_meta.get("path"))
                code_blocks.append(code_block)
            except ValidationError as e:
                errors.add("Failed to create CodeBlock", exception=str(e), error_type=ParseErrorType.PYDANTIC_VALIDATION_ERROR)
    ...
    def _parse_front_matter(self, md_text: str) -> Errors:
        ...
        if match:
            ...
            try:
                ...
                errors.add("Invalid front matter", yaml_str=yaml_str, error_type=ParseErrorType.INVALID_FORMAT)
        ...
        if yaml_dict:
            try:
                ...
                errors.add("Invalid front matter", yaml_dict=yaml_dict, exception=str(e), errors=e.errors(), error_type=ParseErrorType.PYDANTIC_VALIDATION_ERROR)
